...
async function listBookmarks(db, options) {
...
__name(listBookmarks, "listBookmarks");
async function updateBookmark(db, id, data) {
  const timestamp = now();
  ...
  if (data.is_read !== void 0) {
    updates.push("is_read = ?");
    params.push(data.is_read);
  }
  params.push(id);
  await db.prepare(`UPDATE bookmarks SET ${updates.join(", ")} WHERE id = ?`).bind(...params).run();
...
tagsRouter.post("/", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO tags (id, name, color) VALUES (?, ?, ?)").bind(id, body.name.trim(), body.color || "").run();
...
searchRouter.get("/", async (c) => {
  ...
  for (const row of result.results) {
    const tags = await c.env.DB.prepare(`SELECT t.* FROM tags t JOIN bookmark_tags bt ON bt.tag_id = t.id WHERE bt.bookmark_id = ?`).bind(row.id).all();
    bookmarks.push({ ...row, tags: tags.results });
  }
  return c.json({ bookmarks });
});

// src/handlers/fetch-meta.ts
var fetchMetaRouter = new Hono2();
fetchMetaRouter.post("/", async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      cover_image: image,
      source: hostname,
      content: extractTextContent(html).slice(0, 2e3)
      // First 2000 chars for AI
    });
  } catch (e) {
    return c.json({
      title: url,
...
function decodeHtmlEntities(text) {
  return sanitizeText(
    text.replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&#39;/g, "'").replace(/&#x27;/g, "'").replace(/&#x2F;/g, "/").replace(/&#96;/g, "`").replace(/&#x60;/g, "`").replace(/&nbsp;/g, " ")
  );
}
__name(decodeHtmlEntities, "decodeHtmlEntities");

// src/sensenova.ts
async function callSenseNova(apiKey, systemPrompt, userContent) {
...
__name(callSenseNova, "callSenseNova");
var LINK_EXTRACT_PROMPT = `\u4F60\u662F\u4E00\u4E2A\u4E13\u4E1A\u7684\u4FE1\u606F\u6574\u7406\u52A9\u624B\u3002\u6839\u636E\u63D0\u4F9B\u7684\u7F51\u9875\u6807\u9898\u548C\u5185\u5BB9\uFF0C\u751F\u6210\u4EE5\u4E0B\u4E09\u90E8\u5206\uFF1A
...
var NOTE_EXTRACT_PROMPT = `\u6839\u636E\u63D0\u4F9B\u7684\u6587\u5B57\u5185\u5BB9\uFF0C\u751F\u6210\uFF1A
1. \u4E00\u4E2A\u7B80\u6D01\u7684\u6807\u9898\uFF0815\u5B57\u4EE5\u5185\uFF09
2. \u4E00\u6BB5\u7B80\u77ED\u7684\u6458\u8981\uFF0850\u5B57\u4EE5\u5185\uFF09
...
// src/handlers/ai-extract.ts
var aiExtractRouter = new Hono2();
...
// .wrangler/tmp/bundle-1Ndoax/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1Ndoax/middleware-loader.entry.ts
